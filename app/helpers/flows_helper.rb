...
module FlowsHelper
    ...
    def self.output_content (drop)
        ...
        content.scan(%r{(\G|<br/>)(&lt; /code(s)?/(\d+)(<br/>)?)}).uniq.each {|match|
            content.gsub!(/#{Regexp.escape(match[1])}/, ActionView::Base.new(Rails::Configuration.new.view_path).render(:partial => 'codes/show', :locals => { :code => Code.find(match[3]), :inDrop => true }).strip)
        }
