...
module ApplicationHelper
    ...
    def filter_output (content)
        ...
        content.scan(%r{(\G|<br/>)(&lt; (http://#{DOMAIN})?/code(s)?/(\d+)(<br/>)?)}).uniq.each {|match|
            content.gsub!(/#{Regexp.escape(match[1])}/, ActionView::Base.new(Rails::Configuration.new.view_path).render(:partial => "#{theme_path}/codes/show.html.erb", :locals => { :code => (Code.find(match[4]) rescue nil), :outside => true }).strip)
        }
